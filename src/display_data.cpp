...
// 
// CHANGEMENTS v1.03:
// - Ajout dithering pour dégradé fond
// - Bandeau bleu réduit à 34px (28px texte + 6px marge)
// - 7 cadres ombrés style display_gauges.cpp
...
void createDataScreen(const char* boat_name, const char* version)
{
    ...
    lv_obj_set_style_pad_all(wind_frame, 0, 0);
    ...
    // ========================================
    // RÉTICULE (croix au centre) - BARRE HORIZONTALE UNIQUEMENT
    ...
    // ========================================
    // GRADUATIONS RADIALES (36 traits tous les 10°)
    // Par-dessus les arcs de couleur et le réticule
    // ========================================
    ...
    for (int i = 0; i < 36; i++) {
        ...
        bool is_major = (angle % 30 == 0);  // Graduation principale tous les 30°
    ...
    lv_obj_set_style_pad_all(cog_frame, 0, 0);
    ...
    // ========================================
    // GRADUATIONS RADIALES COMPAS COG (36 traits tous les 10°)
    ...
    // Police Montserrat 24, blanc, positionnées au rayon des graduations (128px)
    // ========================================
    struct Cardinal {
        int angle;
        const char* letter;
    ...
    const int cardinal_radius = compass_radius;  // 128px (au rayon des graduations)
    
    for (int i = 0; i < 4; i++) {
        float angle_rad = (cardinals[i].angle - 90) * PI / 180.0f;
        
        // Position au rayon des graduations
    ...
    lv_obj_set_style_pad_all(deepth_frame, 0, 0);
    ...
    lv_obj_set_style_pad_all(gwd_frame, 0, 0);
    ...
    lv_obj_set_style_pad_all(soc_frame, 0, 0);
    ...
    lv_obj_set_style_pad_all(amp_frame, 0, 0);
    ...
    Serial.println("[Data] ✓ Écran principal créé (7 cadres)");
}
